...
export default function HomePage() {
  ...
  const currentlyReading = activeBorrowings[0]
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-12">
      ...
      <section className="relative rounded-3xl overflow-hidden shadow-sm border border-primary-100 mb-10 bg-slate-900">
        <img src="/hero-banner.png" alt="" className="absolute inset-0 w-full h-full object-cover opacity-80 mix-blend-luminosity" />
      ...
      <section className="mb-16">
        ...
        {currentlyReading ? (
          <div className="relative bg-surface-container-highest rounded-[1.5rem] p-6 ring-1 ring-outline-variant/15 flex flex-col sm:flex-row gap-6 items-start mt-10">
            {/* Overlapping Image */}
            <div className="-mt-12 flex-shrink-0 z-10 shadow-lg shadow-on-surface/5">
              <BookCover book={currentlyReading.books} size="md" className="ring-1 ring-outline-variant/30" />
            </div>
          ...
          <div className="bg-surface-container-low rounded-2xl p-8 text-center ring-1 ring-outline-variant/15">
            <span className="material-symbols-outlined text-4xl text-primary-300 mb-3 block" style={{ fontVariationSettings: "'FILL' 1" }}>menu_book</span>
            <p className="text-on-surface-variant font-body font-medium">Belum ada buku yang sedang dipinjam.</p>
            <Link to="/catalog" className="inline-flex items-center gap-2 mt-4 text-primary font-bold hover:underline">
              <span>Cari Buku</span>
      ...
      <section className="mb-14">
        ...
        <div className="flex overflow-x-auto gap-5 pb-4 snap-x snap-mandatory no-scrollbar -mx-4 px-4 sm:mx-0 sm:px-0">
          {(booksData?.data || []).map((book) => (
            <Link key={book.id} to={`/books/${book.id}`} className="snap-start flex-shrink-0 w-44 bg-surface-container-low rounded-xl p-4 flex flex-col items-center text-center relative mt-6 ring-1 ring-outline-variant/15 hover:bg-surface-container transition-colors duration-300 group">
              <div className="-mt-10 z-10 group-hover:-translate-y-1 transition-transform duration-300">
                <BookCover book={book} size="md" className="shadow-md shadow-on-surface/5" />
              </div>
          ...
          {(!booksData?.data || booksData.data.length === 0) && (
            <div className="w-full text-center py-8">
              <span className="material-symbols-outlined text-4xl text-primary-200 mb-2 block">library_books</span>
      ...
      <section className="mb-8">
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="bg-surface-container-low rounded-[1.25rem] p-4 flex gap-5 items-center ring-1 ring-outline-variant/10 hover:bg-surface-container transition-colors duration-300 cursor-pointer">
            <div className="w-16 h-16 rounded-lg overflow-hidden flex-shrink-0 bg-gradient-to-br from-primary-100 to-primary-200 flex items-center justify-center">
               <span className="material-symbols-outlined text-primary-600 text-2xl" style={{ fontVariationSettings: "'FILL' 1" }}>campaign</span>
            </div>
          ...
          <div className="bg-surface-container-low rounded-[1.25rem] p-4 flex gap-5 items-center ring-1 ring-outline-variant/10 hover:bg-surface-container transition-colors duration-300 cursor-pointer">
            <div className="w-16 h-16 rounded-lg overflow-hidden flex-shrink-0 bg-gradient-to-br from-amber-50 to-amber-100 flex items-center justify-center">
               <span className="material-symbols-outlined text-amber-600 text-2xl" style={{ fontVariationSettings: "'FILL' 1" }}>groups</span>
            </div>
            <div className="flex-1">
